...
public class UILabel : UIWidget
{
	...
	public override void OnFill (BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols)
	{
		...
		if(effectStyle == Effect.Shadow_Outline)
		{
			int end = verts.size;
//			NGUIText.Print(text, verts, uvs, cols, effectStyle == Effect.Outline2);
			ApplyBoldOutline(text, verts, uvs, cols, offset, end, mEffectDistance.x, mEffectDistance.y);
		...
		if (effectStyle != Effect.None && effectStyle != Effect.Shadow_Outline)
		{
			int end = verts.size;
	...
	public void ApplyBoldOutline(string text, BetterList<Vector3> verts, BetterList<Vector2> uvs, BetterList<Color32> cols, int start, int end, float shadow_x, float shadow_y)
	{
		if (string.IsNullOrEmpty(text)) return;

		int indexOffset = verts.size;
		NGUIText.Prepare(text);
		float[] mBoldOffset = new float[] { -0.5f, 0f, 0.5f, 0f, 0f, -0.5f, 0f, 0.5f };
		int ch = 0, prev = 0;
		...
		for (int i = 0; i < text.Length; ++i)
		{
			...
			if (ch == '\n')
			{
				...
				{
					NGUIText.Align(verts, indexOffset, x - NGUIText.finalSpacingX);
			...
			float v0x = glyph.v0.x + x;
			float v0y = glyph.v0.y - y;
			float v1x = glyph.v1.x + x;
			float v1y = glyph.v1.y - y;

			// Advance the position
			x += NGUIText.finalSpacingX + glyph.advance;
			...
			for (int j = 0; j < 4; ++j)
			{
				...
				verts.Add(new Vector3(v0x + a, v1y + b));
				verts.Add(new Vector3(v1x + a, v1y + b));
		...
		// DUplicate Shadow
		float shadow_x1 = Math.Abs(shadow_x);
		float shadow_y1 = Math.Abs(shadow_y);
		float shadow_max = (shadow_x1 > shadow_y1) ? shadow_x1 : shadow_y1;
		int shadow_index = (end - start) * 4;

		// Shadow 거리가 1보다 작을경우 한번만 더 그려줌
		if (shadow_max < 1)
		{
			for (int i = 0; i < shadow_index; ++i)
			{
				verts.Add(verts.buffer[end + i]);
				uvs.Add(uvs.buffer[end + i]);
				cols.Add(cols.buffer[end + i]);

				Vector3 v = verts.buffer[end + shadow_index + i];
				v.x += shadow_x;
				v.y += shadow_y;
				verts.buffer[end + shadow_index + i] = v;

				uc = cols.buffer[end + shadow_index + i];

				if (uc.a == 255)
				{
					cols.buffer[end + shadow_index + i] = col;
				}
				else
				{
					Color fc = c;
					fc.a = (uc.a / 255f * c.a);
					cols.buffer[end + shadow_index + i] = (bitmapFont != null && bitmapFont.premultipliedAlphaShader) ? NGUITools.ApplyPMA(fc) : fc;
				}
			}
		}
		else
		{
			for (int j = 0; j < shadow_max; ++j)
			{
				for (int i = 0; i < shadow_index; ++i)
				{
					verts.Add(verts.buffer[end + shadow_index * j + i]);
					uvs.Add(uvs.buffer[end + shadow_index * j + i]);
					cols.Add(cols.buffer[end + shadow_index * j + i]);

					Vector3 v = verts.buffer[end + shadow_index * (j + 1) + i];
					v.x += shadow_x / shadow_max;
					v.y += shadow_y / shadow_max;
					verts.buffer[end + shadow_index * (j + 1) + i] = v;

					uc = cols.buffer[end + shadow_index * (j + 1) + i];

					if (uc.a == 255)
					{
						cols.buffer[end + shadow_index * (j + 1) + i] = col;
					}
					else
					{
						Color fc = c;
						fc.a = (uc.a / 255f * c.a);
						cols.buffer[end + shadow_index * (j + 1) + i] = (bitmapFont != null && bitmapFont.premultipliedAlphaShader) ? NGUITools.ApplyPMA(fc) : fc;
					}
				}
			}
		}

		// Reverse Drowing Order
	...
	public void ApplyOutline3()
	{
		if (this.gameObject.transform.childCount != 0) return;

		UILabel outline = (UILabel) GameObject.Instantiate(this.gameObject) as UILabel;
		outline.transform.parent = this.gameObject.transform;
		outline.transform.localScale = Vector3.one;
		outline.color = mEffectColor;
		outline.text = "[b]" + outline.text;
		outline.effectStyle = Effect.None;
		outline.depth = this.depth - 1;
	}
