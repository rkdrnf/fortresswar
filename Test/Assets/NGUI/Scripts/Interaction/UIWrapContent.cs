...
public class UIWrapContent : MonoBehaviour
{
    ...
    protected void Reset(bool isResetChildPosition = false)
    {
        if (!CacheScrollView())
        {
            Debug.Log("CacheScrollView false!");
            return;
        }

        if (isResetChildPosition)
        {
            ResetChildPositions();
    ...
    protected void ResetClippingPosition()
    {
        if (mScroll != null)
            mScroll.ResetPosition();
    }

    protected void CheckClippingPosition()
    {
        if (mScroll != null && mScroll.transform.localPosition.y < -3000)
        {
            Debug.Log("Err : CheckClippingPosition");
            mScroll.ResetPosition();
        }
    }
    ...
    public void SetLastChildPosition()
    {
        //maxIndex 와 minIndex를 통해 지금 몇개가 표시되고 있는지 알아내고,
        int itemCount = maxIndex - minIndex + 2;

        //아이템 수가 적을땐 맨 밑으로 스크롤 하는게 의미없음
        if (itemCount < mChildren.Count)
        {
            return;
        }

        //mChild 의 알맞은 localPosition 을 결정해야한다.
        //이후 wrapContent() 를 통해 wrapIndex 가 정해질 것.
        for (int i = 0; i < mChildren.Count; i++)
        {
            Vector3 v = mChildren[i].localPosition;
            v.y = -((itemCount - i) * itemSize);

            mChildren[i].localPosition = v;
        }
